...
class FlaskUtil:
    ...
    def list_pagenation(
        ...
        **kwargs,
        ...
        if not orderby:
            cases = (
                query.limit(per_page_nums)
                .offset(start)
                .order_by(moudle.update_time.desc())
            )
        else:
            cases = query.limit(per_page_nums).offset(start).order_by(orderby)
        ...
        for case in cases:
            ...
            case_list.append(
                model_to_dict(case, exclude=exclude, recurse=recurse, **kwargs)
            )
    ...
    def update_api(self, moudle: BaseModel):
        ...
        try:
            ...
            return JsonResponse.error_response(
                error_message={"msg": f"更新{moudle} id:{id_}失败", "error": str(e)}
            )
    ...
    def create_model_instance(
        ...
        try:
            ...
            return JsonResponse.error_response(
                error_message={"msg": f"创建{model} 失败", "error": str(e)}
            )

        return JsonResponse.success_response(
            data=model_to_dict(instance, exclude=[model.is_deleted], recurse=False),
            msg=f"创建{model} 成功",
        )
